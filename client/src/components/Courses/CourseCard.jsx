...
const CourseCard = ({ course, isEnrolled }) => {
    ...
    return (
            <div className={`bg-white rounded-md ${isEnrolled ? 'min-w-[150px]' : 'min-w-[250px]'} w-fit shadow-lg hover:shadow-sm cursor-pointer  border border-gray-300`} onClick={() => {
                if(!isEnrolled) {
                    navigate(`/course/${course._id}`)
                }
            }}>
                <div className='p-2.5 pb-0 flex items-center justify-center'>
                    <div className={`${isEnrolled ? "w-[150px] h-[80px]" : "w-[250px] h-[150px]"}  flex items-center justify-center bg-black rounded-sm overflow-hidden`}>
                ...
                <div className={`mx-auto p-2 px-0.5 pt-1.5 flex flex-col   ${isEnrolled ? 'w-[150px]' : 'w-[250px] min-h-[110px]'}`}>
                    ...
                    }
                    {
                        isEnrolled && (
                            <div>
                                <button className='mt-3 w-full bg-red-700 text-white px-5 py-1.5 rounded-md text-xs cursor-pointer hover:bg-red-500' onClick={(e) => {
                                    e.stopPropagation()
